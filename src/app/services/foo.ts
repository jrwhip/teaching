      ...
      function generateSimplifyProblem() {
        ...
        const hint = `Divide numerator and denominator by the greatest common factor, which is ${commonFactor}.` +
...
3. Write down the sum with the decimal point aligned`;
    ...
    3. Write down the difference with the decimal point aligned.`;
        ...
        return new Problem(`${num1} - ${num2}`, difference.toString(), validateFn, hint);
      }
    ...
    <table style="border-collapse: collapse;">
      ...
      <tr>
        ...
        <td style="text-align: right;">&nbsp;</td>
         ...
         <td style="text-align: right;">&nbsp;</td>
        <td style="text-align: right;">&nbsp;</td>
        <td style="text-align: right;">${multiplicationStep1 + multiplicationStep2 * 10}</td>
      </tr>
    </table>`;
      ...
      function generateDivisionProblem() {
        ...
        const fractionalString = roundedFractionalPart === 0 ? '' : roundedFractionalPart.toString().slice(1);
    ...
    `;
        ...
        const validateFn = (userInput: string) => {
          ...
          const isCorrect = parsedInput.toFixed(comparisonPrecision) == answer.toFixed(comparisonPrecision);
      ...
      function generateWholeTimesMixedProblem() {
        ...
        const mixedFractionDenominator = mixedFractionNumerator + Math.floor(Math.random() * (8 - mixedFractionNumerator)) + 1;
        ...
        const greatestCommonDivisorM = helper.calculateGCF(productFractionNumerator, productFractionDenominator);
        let simplifiedNumeratorM = productFractionNumerator / greatestCommonDivisorM;
        let simplifiedDenominatorM = productFractionDenominator / greatestCommonDivisorM;
        ...
        const finalAnswer = remainingNumeratorM > 0 ? `${totalWholePart} ${remainingNumeratorM}/${simplifiedDenominatorM}` : `${totalWholePart}`;
...
`;
        ...
        const validateFn = (userInput: string) => {
          ...
          const isCorrect = inputWholeNumber === totalWholePart &&
      ...
      function generateMedianProblem() {
        const numbersForMedian = Array.from({
          length: 5 + Math.floor(Math.random() * 5)
        }, () => Math.floor(Math.random() * 20) + 1).sort((a, b) => a - b);
        const midIndex = Math.floor(numbersForMedian.length / 2);
        const median = numbersForMedian.length % 2 !== 0 ? numbersForMedian[midIndex] : (numbersForMedian[midIndex - 1] + numbersForMedian[midIndex]) / 2;
      ...
      function generateModeProblem() {
        ...
        let frequency = numbersForMode.reduce((acc: { [key: number]: number }, num: number) => {
            acc[num] = (acc[num] || 0) + 1;
            return acc;
        }, {});
        ...
        let mode = Object.keys(frequency).filter(num => frequency[+num] === maxFreq);
      ...
      function generateRoundingProblem() {
        ...
        if (roundingPosition >= 0 && roundingPosition < numStr.length) {
          ...
          nextDigit = roundingPosition + 1 < numStr.length ? numStr[roundingPosition + 1] : '0';
        }
...
`;
      ...
      function generateComparisonQuestion() {
        ...
        if (currentType === 'fraction') {
          ...
          const denominator2 = Math.floor(Math.random() * (9 - numerator2)) + numerator2;
          ...
          answer = helper.compareFractions(numerator, denominator, numerator2, denominator2);
        } else {
      ...
      function generateDivideFractionsProblem() {
        ...
        const simplifiedAnswer = helper.simplifyFraction(productNumerator, productDenominator);
    ...
    <tr>
        ...
        <td>
            ...
            <span class="frac"><sup>${numerator1*denominator2}</sup><span>&frasl;</span><sub>${denominator1*numerator2}</sub></span>
        </td>
        <td>
...
`;
      ...
      function generateMixedToImproperProblem() {
        ...
        const simplified = helper.simplifyFraction(improperNumerator, fractionDenominator);
    ...
    <tr>
        <td>1</td>
        <td>
            ${whole} = <span class="frac"><sup>${whole * fractionDenominator}</sup><span>&frasl;</span><sub>${fractionDenominator}</sub></span>
        </td>
        <td>
    ...
    <tr>
        <td>2</td>
        <td>
            <span class="frac"><sup>${whole * fractionDenominator}</sup><span>&frasl;</span><sub>${fractionDenominator}</sub></span>+<span class="frac"><sup>${fractionNumerator}</sup><span>&frasl;</span><sub>${fractionDenominator}</sub></span> = <span class="frac"><sup>${improperNumerator}</sup><span>&frasl;</span><sub>${fractionDenominator}</sub></span>
        </td>
        <td>
            Add the fractions.
        </td>
...
`;
        ...
        const validateFn = (userInput: string) => {
          ...
          const simplifiedInput = helper.simplifyFraction(parseInt(inputFraction[0], 10), parseInt(inputFraction[1], 10));
          const isCorrect = simplifiedInput.numerator === simplified.numerator && simplifiedInput.denominator === simplified.denominator;
...
`;
    ...
    const gcdResult = helper.calculateGCF(resultFractionNumerator, commonDenominator);
    ...
    const finalGcd = helper.calculateGCF(simplifiedNumerator, simplifiedDenominator);
    ...
    const question = `${originalWhole1}<sup>${fractionNumerator1}</sup>/<sub>${fractionDenominator1}</sub> ${isAddition ? '+' : '-'} ${whole2}<sup>${fractionNumerator2}</sup>/<sub>${fractionDenominator2}</sub>`;


    const hint = `<table border="1" cellpadding="5">
    <tr>
        <td colspan="2">Step ${originaladjustedNumerator1 < adjustedNumerator2 && !isAddition ? '5' : '4'}</td>
        <td>Explanation</td>
    </tr>
    <tr>
        <td>1</td>
    ...
    <tr>
        <td>2</td>
        <td>
             <span class="frac"><sup>${fractionNumerator1}  &bull;  ${commonDenominator/fractionDenominator1}</sup><span>&frasl;</span><sub>${fractionDenominator1}  &bull;  ${commonDenominator/fractionDenominator1}</sub></span> = <span class="frac"><sup>${originaladjustedNumerator1}</sup><span>&frasl;</span><sub>${commonDenominator}</sub></span> <br><br>
             <span class="frac"><sup>${fractionNumerator2}  &bull;  ${commonDenominator/fractionDenominator2}</sup><span>&frasl;</span><sub>${fractionDenominator2}  &bull;  ${commonDenominator/fractionDenominator2}</sub></span> = <span class="frac"><sup>${adjustedNumerator2}</sup><span>&frasl;</span><sub>${commonDenominator}</sub></span>
        <td>
    ...
    <tr>
        <td>3</td>
        <td>
            ${isAddition ? '' : originaladjustedNumerator1 < adjustedNumerator2 ? 'Borrow from the whole and add it to the numerator <br><br>'+ originalWhole1 + ' :' : ''}<br>
            <span class="frac"><sup>${adjustedNumerator1}</sup><span>&frasl;</span><sub>${commonDenominator}</sub></span> ${isAddition ? '+' : '-'} <span class="frac"><sup>${adjustedNumerator2}</sup><span>&frasl;</span><sub>${commonDenominator}</sub></span>
        </td>
        <td>
            ${isAddition ? 'Add' : 'Subtract'} the numerators, while the denominator remains the same.
        </td>
    </tr>
    ...
    <tr>
        <td>4</td>
        <td>
            ${isAddition ? 'Add' : 'Subtract'} the borrowed whole number from ${originalWhole1} and update the fraction accordingly.<br><br>
            New fraction: <span class="frac"><sup>${adjustedNumerator1 - commonDenominator}</sup><span>&frasl;</span><sub>${commonDenominator}</sub></span>
        </td>
        <td>
            Adjust the fraction after borrowing and ${isAddition ? 'addition' : 'subtraction'}.
        </td>
    </tr>` : ``}
    <tr>
...
`;
    ...
    return new Problem(question, answer, validateFn, hint);
      ...
      function generateSolveForXProblem() {
        ...
        const hint = `1. Substitute x with ${num2} into the equation: ${num1}(${num2}) ${operation} ${num3}.<br>` +
      ...
      function generateExponentProblem() {
        ...
        const hint = `To calculate ${num1}<sup>${exponent}</sup>, multiply ${num1} by itself ${exponent} times. ` +
      ...
      function generateLCMProblem() {
        ...
        return new Problem(`Find the LCM of ${num1} and ${num2}`, lcm.toString(), validateFn, hint);
      }
      ...
      function generateGCFProblem() {
        ...
        const hint = `The Greatest Common Factor (GCF) of two numbers is the largest number that divides both of them without leaving a remainder. For ${num1*num2*num3} and ${num1*num2*num4}, first identify the common factors.`;
        ...
        return new Problem(`Find the GCF of ${num1*num2*num3} and ${num1*num2*num4}`, gcf.toString(), validateFn, hint);
      }
      ...
      function generateHintForUnknownType(type, num1, num2, num3) {
        ...
        Then subtract this from the total (${num3} x ${num1+num2}) to find the missing part.
        ...
        This tells you the combined total of ${num1} and ${num2} before multiplying by ${num3}.`;
                                break;
                            case 4:
                                hint = `(${num2} + ${num3}) = ${num2 + num3} add first since it's in parentheses.
      ...
      function generateFactorProblem() {
        ...
        if (isFactoring) {
          ...
          answer = `${commonFactor}( ${num1 / commonFactor}n - ${num2 / commonFactor} )`;
          hint = `To factor the expression, first identify the greatest common factor (GCF) of the coefficients. 
            \nFor ${commonFactor * num1}n and ${commonFactor * num2}, the GCF is ${commonFactor}. 
            ...
            \n${commonFactor}(${num1 / commonFactor}n - ${num2 / commonFactor}).`;
        } else {
          ...
          num2 = num3 * (Math.floor(Math.random() * 20) + 2);
          const commonFactor = helper.calculateGCF(num1, num2);
          question = `Use the GCF to simplify: ${commonFactor * num1} + ${commonFactor * num2}`;
          answer = `${commonFactor}( ${num1 / commonFactor} + ${num2 / commonFactor} )`;
          hint = `Simplify by identifying the GCF of the terms, which is ${commonFactor}. Then express the sum as a product of the GCF and a simplified expression: 
            \n${commonFactor}(${num1 / commonFactor} + ${num2 / commonFactor}).`;
        }
      ...
      function generateDistributeProblem() {
        ...
        const hint = `To distribute, multiply the number outside the parentheses by each term inside the parentheses: ` +
          ...
          `<br>    ${coefficient*term1}n - ${coefficient*term2}.    Remember to keep the sign.`;
  ...
  Find the pattern or ratio that fits these columns to solve for the missing numbers.`;
        ...
        const validateFn = (userInput: string) => {
          ...
          const isCorrect = parsedInput.length === 2 &&
      ...
      function generateDivideMixedNumbersProblem() {
        ...
        const fractionNumerator1 = Math.floor(Math.random() * 7) + 1;
        const fractionDenominator1 = fractionNumerator1 + Math.floor(Math.random() * 8) + 1;

        const whole2 = Math.floor(Math.random() * 5) + 1;
        const fractionNumerator2 = Math.floor(Math.random() * 7) + 1;
        const fractionDenominator2 = fractionNumerator2 + Math.floor(Math.random() * 8) + 1;
...
`;
 ...
 // Adjust according to your feedback mechanism
        ...
        return new Problem(question, `${simplifiedAnswer.numerator}/${simplifiedAnswer.denominator}`, validateFn, hint);
      }
      ...
      function generateExponent2Problem() {
        ...
        if (wasFraction) {
          ...
          const simplifiedFraction = helper.simplifyFraction(numeratorExponentiated, denominatorExponentiated);
          ...
          hint = `To calculate ${base}<sup>${exponent}</sup>, multiply ${base} by itself ${exponent - 1} times. Remember, the exponent tells you how many times to use the base as a factor.`;
        }
